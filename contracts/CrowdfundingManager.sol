...
contract CrowdfundingManager {
    ...
    mapping(address user => mapping(uint campaignId => uint amountFunded))
        public s_allocatedFunds;
    ...
    function createCampaign(uint _expirationDate, uint _prizeGoal) public {
        ...
        require(
            ...
            "CrowdfudingManager: invalid expiration date"
        );
    ...
    function fundCampaign(uint campaignId, uint amount) public {
        ...
        require(
            ...
            "CrowdfudingManager: Campaign dosen't exist"
        );
        require(
            campaign.expirationDate > block.timestamp,
            "CrowdfudingManager: campaign expired"
        );
    ...
    function collectFunds(uint campaignId) external {
        ...
        require(
            ...
            "CrowdfudingManager: you're not the creator"
        );
        require(
            campaign.isOngoing == false,
            "CrowdfudingManager: is not expired yet"
        );
    ...
    function withdrawFunds(uint campaignId) external {
        ...
        require(
            campaign.isOngoing == false,
            "CrowdfudingManager: campaign is not expired yet"
        );
        require(
            campaign.prizeGoal > campaign.amountRaised,
            "CrowdfundingManager: campaign was succesful"
        );
